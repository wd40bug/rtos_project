...
static bool i2c_wait_flag_set(uint32_t flag_mask) {
  uint32_t start = (uint32_t)ms_since_start();
  while ((I2C1->ISR & flag_mask) == 0U) {
...
static bool i2c_wait_flag_clear(uint32_t flag_mask) {
  uint32_t start = (uint32_t)ms_since_start();
  while ((I2C1->ISR & flag_mask) != 0U) {
    if (((uint32_t)ms_since_start() - start) > I2C_TIMEOUT_MS) {
      return false;
...
static void i2c_clear_errors_and_stop() {
...
static I2C_Error i2c_check_errors() {
  uint32_t isr = I2C1->ISR;

  if (isr & (I2C_ISR_BERR | I2C_ISR_ARLO | I2C_ISR_NACKF)) {
    i2c_clear_errors_and_stop();
    return INVALID_I2C;
  }
...
I2C_Error i2c_write(uint8_t slave_addr, uint8_t* data, uint8_t data_len) {
  ...
  if (!i2c_wait_flag_clear(I2C_ISR_BUSY)) {
    return INVALID_I2C;
  }
  ...
  for (uint8_t i = 0; i < data_len; i++) {
    while ((I2C1->ISR & I2C_ISR_TXE) == 0U) {
      I2C_Error err = i2c_check_errors();
      if (err != I2C_OK) {
        return err;
      }
    }
  ...
  // Wait for STOPF
  if (!i2c_wait_flag_set(I2C_ISR_STOPF)) {
    return INVALID_I2C;
  }
...
// Blocking read
I2C_Error i2c_read(
    uint8_t slave_addr, uint8_t* data, uint8_t data_len
) {
  if (data_len == 0U) {
    return I2C_OK;
  }
...
I2C_Error i2c_write_reg(
  ...
  val.data = data;
  uint8_t write[3] = {reg_addr, val.u8s[0], val.u8s[1]};

  return i2c_write(slave_addr, write, 3);
}
...
I2C_Error i2c_read_reg(
  ...
  I2C_Error err = i2c_write(slave_addr, &reg_addr, 1U);
  if (err != I2C_OK) {
    return err;
  }

  return i2c_read(slave_addr, (uint8_t*)data, 2);
}
